...
function UsersTab() {
  ...
  if (selectedShops.length === 0) throw new Error('Seleziona almeno un negozio per l\'utente.')
...
const handleResendInvite = async (email: string) => {
  try {
    ...
    toast({title:"Email di attivazione inviata con successo!", variant:'default'});
  } catch (err: any) {
    toast({title:'Errore', description:"Errore nel reinvio: " + err.message, variant:'destructive'});
  }
};
    ...
    <Card>
      <div className="flex justify-between items-center mb-6">
        <div className="flex flex-row items-center gap-2 w-full md:w-auto">
          <h3 className="font-bold text-base sm:text-lg whitespace-nowrap">Gestione Utenti</h3>
        ...
        <Button onClick={() => openModal()} className="px-3">
          <Plus className="shrink-0 w-4 h-4 mr-2" /> 
          <span className="hidden md:inline">Nuovo Utente</span>
          <span className="inline md:hidden">Utente</span>
        </Button>
      </div>
                      ...
                      onClick={async () => {
                        if (selectedUserId) {
                          try {
                            ...
                            toast({title:'Errore', description:`Errore durante il blocco/sblocco:${err}`, variant:'destructive'});
                          }
                        }
                      }}
                    >
